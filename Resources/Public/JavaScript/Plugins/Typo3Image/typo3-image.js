...
function getImageDialog(editor, img, attributes) {
    ...
    d.$el = $('<div class="rteckeditorimage form-section">');
    ...
    $.each(fields, function () {
        ...
        $.each(this, function (key, config) {
            var $group = $('<div class="form-group mb-3">').appendTo($('<div class="col-xs-12 col-sm-6">').appendTo($row));
            ...
            if (config.type === 'text') {
                ...
                cboxLabel = $('<label>', {
                    ...
                    for: 'checkbox-' + key,
                    text: hasDefault
                        ? img.lang.override.replace('%s', img[key])
                        : img.lang.overrideNoDefault
                });

                // Create a form-check div and append elements properly
                var checkboxWrapper = $('<div>', {
                    class: 'form-check',
                    css: { margin: '0 0 6px' }
                }).append(cbox, cboxLabel);

                // Append to parent group
                $group.append(checkboxWrapper);

                // Click event to toggle input field state
    ...
    var $checkboxTitle = d.$el.find('#checkbox-title'),
        ...
        $zoom = $('<input id="checkbox-zoom" class="form-check-input" type="checkbox">'),
        ...
        var $checkboxZoom = $('<div class="form-check mb-3">').append(
            $zoom,
            $('<label class="form-check-label" for="checkbox-zoom">').text(img.lang.zoom) // Replace with img.lang.zoom if needed
        );
        
        // Append it to the desired location
        $checkboxZoom.appendTo($customRowCol1);
    $inputCssClass
        .prependTo(
            $('<div class="form-group mb-3">').prependTo($customRowCol2)
        )
...
function askImageAttributes(editor, img, attributes, table) {
    ...
    const modal = Modal.advanced({
        ...
        additionalCssClasses: [`ckeditor--modal ckeditor--modal-${Modal.sizes.small}`],
        content: dialog.$el,
        buttons: [
            {
                text: TYPO3.lang['NsRteCkeditorPack.confirmModal.InsertImage'] || 'Insert Image',
...
function selectImage(editor) {
    ...
    const modal = Modal.advanced({
        ...
        additionalCssClasses: [`ckeditor--modal ckeditor--modal-${Modal.sizes.large}`],
