...
if [ "${CONTAINER_BIN}" == "docker" ]; then
    if [[ -n "${CONTAINER_INTERACTIVE}" ]]; then
        CONTAINER_COMMON_PARAMS+=(-it --init)
    fi
    CONTAINER_COMMON_PARAMS+=(--rm --network "${NETWORK}" --add-host "${CONTAINER_HOST}:host-gateway")
    if [[ -n "${USERSET}" ]]; then
        CONTAINER_COMMON_PARAMS+=(--user "${HOST_UID}")
    fi
    CONTAINER_COMMON_PARAMS+=(-v "${ROOT_DIR}:${ROOT_DIR}" -w "${ROOT_DIR}")
else
    # podman
    ...
    if [[ -n "${CI_PARAMS}" ]]; then
        # Intentional word-splitting for CI-provided extra runtime args (for example "--memory=6g").
        read -r -a CI_PARAMS_ARRAY <<< "${CI_PARAMS}"
        CONTAINER_COMMON_PARAMS+=("${CI_PARAMS_ARRAY[@]}")
    fi
    CONTAINER_COMMON_PARAMS+=(--rm --network "${NETWORK}" -v "${ROOT_DIR}:${ROOT_DIR}" -w "${ROOT_DIR}")
fi
    ...
    functional)
        ...
        case ${DBMS} in
            ...
            sqlite)
                ...
                ${CONTAINER_BIN} run "${CONTAINER_COMMON_PARAMS[@]}" --name functional-${SUFFIX} ${XDEBUG_MODE} -e XDEBUG_CONFIG="${XDEBUG_CONFIG}" -e typo3DatabaseDriver=pdo_sqlite --tmpfs "${ROOT_DIR}/typo3temp/var/tests/functional-sqlite-dbs/:rw,noexec,nosuid" ${IMAGE_PHP} "${COMMAND[@]}"
                SUITE_EXIT_CODE=$?
                ;;
        esac
        ;;
